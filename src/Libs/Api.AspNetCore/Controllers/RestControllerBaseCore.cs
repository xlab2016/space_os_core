...
{
    public abstract class RestControllerBaseCore<T, TKey, TDto, TQuery, TMap> : ControllerBase
        ...
        where TDto : class
    {
        ...
        protected RestControllerBaseCore(ILogger<RestServiceBase<T, TKey>> logger, IDapperDbContext restDapperDb, DbContext restDb, string tableName,
            TMap map)
        {
            this.logger = logger;
        ...
        protected async Task<PagedList<TDto>> SearchAsync(TQuery query)
        {
        ...
        protected async Task<TDto> FindAsync(TKey key)
        {
        ...
        protected async Task<TKey> AddAsync(TDto request)
        {
        ...
        protected async Task UpdateAsyncCore(T source)
        {
        }

        protected async Task<bool> UpdateAsync(TDto request)
        {
            var source = map.ReverseMap(request);
